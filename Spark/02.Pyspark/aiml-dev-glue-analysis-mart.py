...
S3_location = 's3://aiml-dev-anomaly-detection/model-data/'
cols_to_update = ['errorlabel']

# 하루전날 까지 error label을 붙히는 함수
...
# join시 version 정보가 없는경우가있어 디센딩시키고, 버전있는경우를 찾아 append
def version(temp):
  import pandas as pd
  import copy
...
ehrd002_write = ehrd002_df_mac_address_list.write.format('csv').mode('overwrite').option('header','true').save('s3://aiml-dev-macaddress-list/macaddress_list/macaddress_002/')
...
ehrd003_write = ehrd003_df_mac_address_list.write.format('csv').mode('overwrite').option('header','true').save('s3://aiml-dev-macaddress-list/macaddress_list/macaddress_003/')
...
alive002_standard_deviation = spark.read.format('csv').option('header','True').load('s3://aiml-dev-macaddress-list/macaddress_list/macaddress_002/')
alive003_standard_deviation = spark.read.format('csv').option('header','True').load('s3://aiml-dev-macaddress-list/macaddress_list/macaddress_003/')
...
# resultDF = version_labelDF.selectExpr('*','cast(additionalvalue as string)')
resultDF=resultDF.withColumn("additionalvalue", resultDF["additionalvalue"].cast("string"))
...
# resultDF.write.format('csv').option('header','true').save(S3_location)
